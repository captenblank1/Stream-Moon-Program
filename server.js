// server.js - الإصدار النهائي المعدل (keystroke ليس نوعاً مستقلاً، بل حقل combo اختياري)
// وإعدادات RCON و TikTok منفصلة لكل مستخدم
...
// تخزين مؤقت لرموز الربط (صلاحية 5 دقائق)
let bindingTokens = new Map(); // key: token, value: { userId, expires }

// أضف هذا السطر مع المتغيرات العامة في بداية الملف
...
async function executeNativeKeystroke(keys, repeat = 1, intervalMs = 500) {
  ...
  const nircmdPath = path.join(__dirname, "nircmd.exe");
  // تحويل الصيغة: "Ctrl+Shift+A" -> "ctrl+shift+a"
  let nircmdKeys = keys.toLowerCase().replace(/\+/g, "+");

  const sendOnce = () => {
    return new Promise((resolve) => {
      exec(
        `"${nircmdPath}" sendkeypress ${nircmdKeys}`,
        (error, stdout, stderr) => {
          if (error) {
    ...
    Audio.findOne({ file })
      .then((audio) => {
        if (audio && audio.cloudinaryUrl) {
          res.redirect(audio.cloudinaryUrl);
        } else {
          const cloudinaryUrl = `https://res.cloudinary.com/${process.env.CLOUDINARY_CLOUD_NAME}/raw/upload/${encodeURIComponent(file)}`;
...
app.use("/videos", (req, res, next) => {
  const file = req.path.substring(1);
  if (file) {
    const cloudinaryUrl = `https://res.cloudinary.com/${process.env.CLOUDINARY_CLOUD_NAME}/video/upload/${encodeURIComponent(file)}`;
    res.redirect(cloudinaryUrl);
  } else {
    next();
  }
  ...
  .then(async () => {
    ...
    try {
      ...
      if (usersCollectionExists) {
        ...
        logger.info("ℹ️ تم إزالة حقل username من المستندات");
        ...
        for (const indexName of oldIndexNames) {
          try {
            ...
            logger.info(`✅ تم حذف الفهرس القديم ${indexName}`);
          } catch (err) {
        ...
        try {
          ...
          logger.info("✅ تم التأكد من وجود فهرس فريد على email");
          await db
            .collection("users")
            .createIndex({ screenToken: 1 }, { unique: true, sparse: true });
          logger.info("✅ تم إنشاء فهرس فريد لـ screenToken");
        } catch (err) {
          logger.warn("⚠️ فشل إنشاء فهارس:", err.message);
        }
      }

      const profilesExists = collections.some((c) => c.name === "profiles");
      if (profilesExists) {
        try {
          const indexes = await db.collection("profiles").indexes();
          const oldIdIndex = indexes.find((idx) => idx.name === "id_1");
          if (oldIdIndex) {
            await db.collection("profiles").dropIndex("id_1");
            logger.info("✅ تم حذف الفهرس القديم id_1 من مجموعة profiles");
          }
        } catch (err) {
          logger.warn("⚠️ فشل حذف الفهرس القديم id_1:", err.message);
        }
      }
    } catch (err) {
      logger.error("❌ خطأ أثناء تنظيف الفهارس:", err.message);
    }

    try {
      await seedGiftsIfNeeded();
    } catch (e) {
      logger.warn("⚠️ خطأ أثناء التهيئة:", e.message);
    }
  })
...
const giftCommandSchema = new mongoose.Schema(
  {
    ...
    combo: { type: String, default: null }, // <-- أضف هذا السطر
  },
  { timestamps: true },
);
...
// ================ التعديل: إزالة "keystroke" من enum ================
const interactionCommandSchema = new mongoose.Schema(
  {
    ...
    combo: { type: String, default: null, index: true }, // حقل اختياري للاختصار
    name: { type: String, default: "" },
    command: { type: String, default: "" },
    webhookUrl: { type: String, default: "" },
    repeat: { type: Number, default: 1 },
  ...
  if (count === 0) {
    ...
    for (let i = 1; i <= MAX_PROFILES; i++) {
      if (!existingIds.includes(i)) {
        ...
        logger.info(`✅ تم إنشاء البروفايل المفقود ${i} للمستخدم ${userId}`);
      }
    }
  }
}
...
async function saveSetting(key, value) {
  try {
    ...
    logger.error("❌ فشل حفظ الإعداد:", key, err.message);
  }
}
async function loadSetting(key, defaultValue = null) {
  try {
    ...
    logger.error("❌ فشل تحميل الإعداد:", key, err.message);
    return defaultValue;
  }
}
...
async function seedGiftsIfNeeded() {
  ...
  if (count === 0) {
    ...
    if (fs.existsSync(giftFile)) {
      ...
      logger.info(
        "ℹ️ لم يتم العثور على gifts.json، سيتم إنشاء الهدايا لاحقاً عبر API",
      );
    }
  }
}
...
async function refreshCachesForUser(userId) {
  try {
    const keysToDelete = [];
    for (const key of giftCommandsCache.keys()) {
      if (key.startsWith(`${userId}:`)) keysToDelete.push(key);
    }
    keysToDelete.forEach((k) => giftCommandsCache.delete(k));
    for (const key of interactionCommandsCache.keys()) {
      if (key.startsWith(`${userId}:`)) keysToDelete.push(key);
    }
    keysToDelete.forEach((k) => interactionCommandsCache.delete(k));

    const gifts = await GiftCommand.find({ userId });
    const interactions = await InteractionCommand.find({ userId });
...
function getGiftCommandForProfile(userId, profile, giftIdStr) {
  ...
  console.log(
    `🔍 getGiftCommandForProfile: key=${key}, map exists? ${!!map}, giftIdStr=${giftIdStr}`,
  );
  if (!map) return null;
  const keyStr = String(giftIdStr);
  const result = map.has(keyStr) ? map.get(keyStr) : null;
  console.log(`   - Searching for ${keyStr}, found? ${!!result}`);
  if (!result) {
    const num = Number(giftIdStr);
    if (Number.isFinite(num) && map.has(String(num))) {
      console.log(`   - Found as number ${num}`);
      return map.get(String(num));
    }
  }
  return result;
}
  ...
  Audio.findOne({ file })
    .then((audio) => {
      ...
      if (targetUserId) {
        ...
        io.to(room).emit("play-sound", payload);
        logger.info(`🔊 تم إرسال أمر تشغيل الصوت ${audioUrl} إلى غرفة ${room}`);
      } else {
        io.emit("play-sound", payload);
        logger.info(`🔊 تم إرسال أمر تشغيل الصوت للجميع: ${audioUrl}`);
      }
    })
    .catch((err) => {
...
async function executeAction(cmdObj, triggerUser = "Unknown", userId) {
  ...
  if (delayBefore > 0) {
    logger.info(`⏳ انتظار ${delayBefore} مللي ثانية قبل التنفيذ...`);
    await new Promise((resolve) => setTimeout(resolve, delayBefore));
  }

  // 🔥 تنفيذ الـ Keystroke إذا وُجد
  if (keystrokeText) {
...
async function connectUser(userId, username) {
  ...
  connection.on(WebcastEvent.GIFT, async (data) => {
    try {
      ...
      if (giftType === 1) {
        ...
        const now = Date.now();
        const st = giftStreakState.get(streakKey) || {
          lastRepeat: 0,
          ts: now,
        };
  ...
  async function processGiftDelta({
    ...
    console.log(
      `\n🔔 [GIFT EVENT] userId=${userId}, sender=${sender}, giftId=${giftIdStr}, delta=${delta}, newRepeat=${newRepeat}`,
    );
    try {
      ...
      if (!giftCmd) {
        console.log(`🔍 Gift command not in cache, querying DB...`);
        ...
        if (giftCmd) {
          console.log(`✅ Found in DB, refreshing cache...`);
          await refreshCachesForUser(userId);
        } else {
          console.log(
            `❌ No gift command found in DB for giftId=${giftIdStr}, profile=${userProfile}`,
          );
        }
      } else {
        console.log(`✅ Gift command found in CACHE`);
      }

      // 🔥 أضف هذا السجل لعرض تفاصيل الأمر إذا وُجد
      if (giftCmd) {
        console.log(
          `📄 Command details: active=${giftCmd.active}, command="${giftCmd.command}", combo="${giftCmd.combo}", webhookUrl="${giftCmd.webhookUrl}", targetUser="${giftCmd.targetUser}"`,
        );
      }

      if (giftCmd && (giftCmd.command || giftCmd.webhookUrl || giftCmd.combo)) {
        console.log(
          `🔍 Checking target: giftCmd.targetUser="${giftCmd.targetUser}", sender="${sender}"`,
        );
        const targetOk =
          !giftCmd.targetUser ||
        ...
        if (targetOk) {
          console.log(`🎯 Target matched, preparing to execute...`);
          ...
          cmdObj.keystrokeText =
            cmdObj.command && cmdObj.command.trim() !== ""
              ? cmdObj.command
              : cmdObj.combo || "";
          console.log(`   keystrokeText = "${cmdObj.keystrokeText}"`);
          await executeAction(cmdObj, sender, userId);
        } else {
          console.log(`❌ Target mismatch, skipping execution.`);
        }
      } else {
        console.log(
          `⚠️ No valid command or webhookUrl, or giftCmd is null. Execution skipped.`,
        );
      }

      // باقي الكود (giftInteractions) بدون تغيير...
  ...
  connection.on(WebcastEvent.ROOM_UPDATE, (data) => {
    ...
    if (!prev && newIsLive) {
      ...
      logger.info(`🟢 بدأ البث المباشر للمستخدم ${userId}`);
    }
    if (prev && !newIsLive) {
      resetOncePerLiveForUser(userId);
      logger.info(`🔴 انتهى البث المباشر للمستخدم ${userId}`);
    }
    if (userTikTokConnections.has(userId)) {
...
<head>
  ...
  <title>شاشات OBS الخاصة بك - Black Moon</title>
  <style>
    ...
    .warning { background: #3a1a1a; padding: 15px; border-radius: 8px; margin-top: 20px; border-left: 4px solid #f44336; }
  </style>
</head>
<body>
<div class="container">
  <h1>🎬 شاشات OBS الخاصة بك</h1>
  <div class="subtitle">لديك 10 شاشات فريدة – لا تشارك الروابط مع أي شخص آخر</div>
    ...
    ${screens
      ...
      <div class="screen-card">
        <div class="screen-header">
          ...
          <button class="copy-btn" onclick="copyUrl('${screen.url}')">📋 نسخ الرابط</button>
        </div>
        <div class="url-box">${screen.url}</div>
        <div style="margin-top: 10px; color: #888;">${screen.description}</div>
      </div>
    `,
      )
      .join("")}
  ...
  <div class="instructions">
    <h3>📌 كيفية الإضافة في OBS:</h3>
    <ol style="color: #ddd; line-height: 1.8;">
      <li>أضف مصدر <strong>متصفح (Browser)</strong> جديد</li>
      <li>الصق أي رابط من الروابط أعلاه</li>
      <li>اضبط العرض: <strong>1920</strong> والارتفاع: <strong>1080</strong></li>
      <li>فعّل ✅ "استخدام معدل إطارات مخصص" = 60 إطار/ثانية</li>
      <li>الشاشة شفافة، وستعرض محتواك الخاص تلقائياً</li>
    </ol>
  </div>
  <div class="warning">
    ⚠️ <strong>هام:</strong> هذه الروابط خاصة بك فقط. لا تشاركها مع أي شخص آخر، لأن أي شخص لديه الرابط يمكنه مشاهدة محتوى شاشتك.
  </div>
</div>
<script>
...
app.delete("/api/audio/:filename", authenticateToken, async (req, res) => {
  try {
    ...
    const publicId = `blackmoon_audio/${basePublicId}`;
...
app.delete("/api/video/:filename", authenticateToken, async (req, res) => {
  try {
    ...
    const basePublicId = path.parse(filename).name;
    const publicId = `blackmoon_videos/${basePublicId}`;
...
app.post("/api/tiktok-disconnect", authenticateToken, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    await user.save();
    logger.info(`📌 تم قطع اتصال TikTok للمستخدم ${userId}`);
...
app.delete("/api/tiktok-user", authenticateToken, async (req, res) => {
  try {
    ...
    const user = await User.findById(userId);
    await user.save();
    logger.info(`📌 تم قطع الاتصال وحذف اسم المستخدم للمستخدم ${userId}`);
...
app.get("/api/profiles", authenticateToken, async (req, res) => {
  try {
    ...
    if (plan === "free") {
      const filtered = profiles.filter((p) => p.id === 1);
      return res.json({ success: true, profiles: filtered });
    }
...
app.post("/api/profiles/import", authenticateToken, async (req, res) => {
  try {
    ...
    if (plan === "free") {
      ...
      const totalAfter = currentTotal + newCommandsCount;
      if (totalAfter > 7) {
        return res.status(403).json({
          success: false,
    ...
    for (const cmd of commands) {
      try {
        if (cmd.giftId !== undefined && cmd.giftId !== null) {
          ...
          // تحويل أوامر keystroke القديمة إلى نوع "all" مع تعيين combo
          let finalCmd = { ...cmd };
...
app.post("/api/profile/select", authenticateToken, async (req, res) => {
  try {
    ...
    logger.info(`🎛️ تم التبديل إلى البروفايل ${p} للمستخدم ${user.email}`);
    await refreshCachesForUser(req.user.id);
...
app.post("/api/gift-commands", authenticateToken, async (req, res) => {
  try {
    ...
    const newGift = await GiftCommand.create({
      ...
      profile,
      userId: req.user.id,
      combo: body.combo || null, // <-- أضف هذا السطر
    });
    await refreshCachesForUser(req.user.id);
...
// ================ تعديل: POST /api/interaction-commands للسماح بـ combo ================
...
// ================ تعديل: PUT /api/interaction-commands/:id ================
app.put(
  ...
  async (req, res) => {
    try {
      ...
      // إذا كان هناك combo جديد، تحقق من عدم تكراره (ما عدا نفس الأمر)
      if (
        req.body.combo &&
...
// ================ تعديل: نقطة نهاية تنفيذ الاختصار (keystroke) ================
...
app.post("/api/execute-keystroke", authenticateToken, async (req, res) => {
  try {
    ...
    if (agentSocket && agentSocket.connected) {
      ...
      agentSocket.emit("execute-keys", payload);
      return res.json({
        success: true,
        method: "local_agent",
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // تعريف keystrokeText هنا في النطاق الخارجي
      const keystrokeText =
        cmdObj.command && cmdObj.command.trim() !== ""
          ? cmdObj.command
          : cmdObj.combo || "";
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      // 🔥 التحقق من وجود العميل المحلي (Agent)
      const agentSocket = userLocalAgents.get(req.user.id);
      if (agentSocket && agentSocket.connected) {
        // تحديد النص الذي سيرسله الـ Agent (كيستروك)
        let keystrokeText =
          cmdObj.command && cmdObj.command.trim() !== ""
            ? cmdObj.command
            : cmdObj.combo || "";

        // إرسال الكيستروك إذا كان هناك نص
        if (keystrokeText) {
          for (let t = 0; t < timesToRun; t++) {
            const payload = {
              command: keystrokeText,
        ...
        // لا نخرج هنا، نكمل لتنفيذ باقي الإجراءات
      }

      // 🔄 تنفيذ الإجراءات الأخرى (RCON، ويب هوك، صوت، فيديو) بغض النظر عن الـ Agent
      for (let t = 0; t < timesToRun; t++) {
        const one = {
          ...cmdObj,
          repeat: configuredRepeat,
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const newTotalVideoMB = user.videoUsedMB + fileSizeMB;
      if (newTotalVideoMB > MAX_VIDEO_MB) {
        return res.status(400).json({
          success: false,
      ...
      logger.info(`✅ تم رفع الفيديو إلى Cloudinary: ${videoUrl}`);
      ...
      user.videoUsedMB = newTotalVideoMB;
      await user.save();
...
app.post(
  ...
  async (req, res) => {
    try {
      ...
      const newTotalAudioMB = user.audioUsedMB + fileSizeMB;
      if (newTotalAudioMB > MAX_AUDIO_MB) {
        return res.status(400).json({
          success: false,
      ...
      logger.info(`✅ تم رفع الصوت إلى Cloudinary: ${audioUrl}`);
      ...
      user.audioUsedMB = newTotalAudioMB;
      await user.save();
...
app.get("/api/admin/stats", authenticateToken, isAdmin, async (req, res) => {
  try {
    ...
    const totalCommands = totalGiftCommands + totalInteractionCommands;
  ...
  try {
    ...
    const usersWithDetails = await Promise.all(
      users.map(async (user) => {
        const commandCount = await getTotalCommandsForUser(user._id);
        const isLiveNow =
...
<script>
  ...
  async function loadStats(){
    const data = await fetchWithAuth(\`\${API_BASE}/api/admin/stats\`);
    ...
    document.querySelectorAll(".btn-renew-monthly, .btn-renew-yearly").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        if (confirm(\`تجديد الاشتراك (\${plan === 'monthly' ? 'شهري' : 'سنوي'})؟\`)) {
          const res = await fetchWithAuth(\`\${API_BASE}/api/admin/user/\${id}/renew\`, {
            method: "POST",
    ...
    document.querySelectorAll(".btn-admin").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        if (confirm("ترقية إلى مدير؟")) {
          const res = await fetchWithAuth(\`\${API_BASE}/api/admin/user/\${id}/make-admin\`, { method: "POST" });
    ...
    document.querySelectorAll(".btn-delete").forEach(btn => {
      btn.addEventListener("click", async () => {
        ...
        if (confirm("حذف المستخدم وجميع أوامره؟")) {
          const res = await fetchWithAuth(\`\${API_BASE}/api/admin/user/\${id}\`, { method: "DELETE" });
  ...
  async function loadUsers(){
    const data = await fetchWithAuth(\`\${API_BASE}/api/admin/users\`);
...
// ================ Socket.IO للعملاء المحليين (جديد) ================
...
// ================ نقطة نهاية لتسجيل العميل المحلي ================
...
// نقطة نهاية يستخدمها العميل لاستبدال رمز الربط برمز WebSocket
...
app.get("/agent-auth", async (req, res) => {
  ...
  bindingTokens.set(bindingToken, {
    userId: null, // سيتم تعبئته بعد تسجيل الدخول
    expires: Date.now() + 5 * 60 * 1000,
    callbackPort: port,
    serverUrl: serverUrl,
  });

  // إرسال صفحة HTML تطلب من المستخدم تسجيل الدخول (أو استخدام الجلسة الحالية)
  res.send(`
    <!DOCTYPE html>
    <html lang="ar">
    <head>
      ...
      <style>
        ...
        input { width: 80%; background: #333; color: white; }
    ...
    <body>
      ...
      <script>
        ...
        bindBtn.onclick = async () => {
          ...
          // طلب إنشاء bindingToken مرتبط بالمستخدم
          ...
          // إعادة التوجيه إلى callback العميل
