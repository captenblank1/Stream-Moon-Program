  ...
  .addEventListener("click", async () => {
    ...
    try {
      const res = await fetchWithAuth(`${API_BASE}/api/rcon-config`, {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ host, port: parseInt(port), password, player }),
      });
      const data = await res.json();
      ...
      if (storageNotif) {
        if (
          ...
          storageNotif.textContent =
            "⚠️ مساحة التخزين على وشك النفاد! يرجى حذف بعض الملفات.";
        } else {
...
document.getElementById("send-usertik").addEventListener("click", async () => {
...
async function initWinsPanel() {
  ...
  $("smwWidth").addEventListener("input", () => {
    $("smwPreview").style.minWidth = (parseInt($("smwWidth").value) || 285) + "px";
    saveSettings();
  });
...
async function initListsPanel() {
  ...
  async function getToken() {
    ...
    const res = await fetchWithAuth(`${API_BASE}/api/user/screen-token`);
    const data = await res.json();
    if (data.success) screenToken = data.token;
    return screenToken;
  }
  ...
  window.listsRefreshData = async () => {
    try {
      const token = await getToken();
      if (!token) return;
      document.getElementById("smlLink1").textContent =
        `${API_BASE}/overlay/overlay.html?token=${token}&id=1`;
      document.getElementById("smlLink2").textContent =
        `${API_BASE}/overlay/overlay.html?token=${token}&id=2`;
      const data = await getSettings(token);
...
async function loadHotkeySettings() {
  try {
    ...
    if (data.success && data.hotkeys && data.hotkeys.length > 0) {
      const hk =
        data.hotkeys.find((h) => h.active === true) ||
        data.hotkeys[data.hotkeys.length - 1];
      hotkeySettings = {
        key: hk.key,
        commandId: hk.commandId,
        commandType: hk.commandType,
        active: hk.active !== undefined ? hk.active : true,
      };
    } else {
      hotkeySettings = {
        key: "",
...
async function applyHotkeySettings() {
  ...
  if (keyInput) keyInput.value = hotkeySettings.key || "";
  if (displayEl) displayEl.textContent = hotkeySettings.key || "لم يتم التعيين";
  if (activeCheck) activeCheck.checked = !!hotkeySettings.active;

  if (select) {
    if (hotkeySettings.commandId && hotkeySettings.commandType) {
      const option = select.querySelector(
        `option[data-id="${hotkeySettings.commandId}"][data-type="${hotkeySettings.commandType}"]`,
      );
      if (option) {
  ...
  if (statusEl) {
    if (registered) {
      statusEl.textContent =
        hotkeySettings.active && hotkeySettings.key
          ? `✅ الاختصار نشط: ${hotkeySettings.key}`
          : "⏸️ الاختصار غير نشط";
      statusEl.style.color = "var(--success-color, #4caf50)";
    } else if (hotkeySettings.active && hotkeySettings.key) {
      statusEl.textContent = "⚠️ فشل تسجيل الاختصار في النظام";
      statusEl.style.color = "var(--warning-color, #ff9800)";
    } else {
      statusEl.textContent = "⏸️ لا يوجد اختصار نشط";
...
function clearHotkeyFormFields() {
  ...
  document.getElementById("hotkeyActive").checked = false;
...
async function saveHotkeySettings() {
  ...
  if (!keyInput || !select) {
    if (statusEl) {
      statusEl.textContent = "❌ عناصر الواجهة غير موجودة";
      statusEl.style.color = "var(--error-color, #f44336)";
    }
    return;
  }
  ...
  if (!newKey) {
    if (statusEl) {
      statusEl.textContent = "⚠️ يرجى اختيار مفتاح من لوحة المفاتيح";
      statusEl.style.color = "var(--warning-color, #ff9800)";
    }
  ...
  if (!isValidHotkeyKey(newKey)) {
    if (statusEl) {
      statusEl.textContent = `⚠️ المفتاح "${newKey}" غير مدعوم`;
      statusEl.style.color = "var(--warning-color, #ff9800)";
    }
  ...
  if (!commandId || !commandType) {
    if (statusEl) {
      statusEl.textContent = "⚠️ يرجى اختيار أمر من القائمة";
      statusEl.style.color = "var(--warning-color, #ff9800)";
    }
  ...
  try {
    ...
    if (data.success && data.hotkeys) {
      ...
      if (editingHotkeyId) {
        ...
        if (!oldHotkey) {
          if (statusEl) {
            statusEl.textContent = "❌ الاختصار المطلوب تعديله غير موجود";
            statusEl.style.color = "var(--error-color, #f44336)";
          }
        ...
        if (existingHotkey) {
          if (statusEl) {
            statusEl.textContent = `⚠️ المفتاح "${newKey}" مستخدم بالفعل مع أمر آخر`;
            statusEl.style.color = "var(--warning-color, #ff9800)";
          }
        ...
        if (updateRes.ok) {
          ...
          if (statusEl) {
            statusEl.textContent = `✅ تم تحديث الاختصار: ${newKey}`;
            statusEl.style.color = "var(--success-color, #4caf50)";
          }
          showMessage(`✅ تم تحديث الاختصار من "${oldKey}" إلى "${newKey}"`);

          // ✅ مسح حالة التعديل بعد التحديث
          editingHotkeyId = null;
          clearHotkeyFormFields();
          return;
        } else {
          ...
          if (statusEl) {
            statusEl.textContent = `❌ فشل تحديث الاختصار: ${errorData.message || "خطأ غير معروف"}`;
            statusEl.style.color = "var(--error-color, #f44336)";
          }
          showMessage(
      ...
      if (existingHotkey) {
        if (statusEl) {
          statusEl.textContent = `⚠️ المفتاح "${newKey}" مستخدم بالفعل`;
          statusEl.style.color = "var(--warning-color, #ff9800)";
        }
    ...
    if (saved) {
      if (statusEl) {
        statusEl.textContent = `✅ تم حفظ الاختصار: ${newKey}`;
        statusEl.style.color = "var(--success-color, #4caf50)";
      }
      await updateHotkeyRegistration();
      await loadHotkeyCommands();
      ...
      if (statusEl) {
        statusEl.textContent = "❌ فشل حفظ الإعدادات على السيرفر";
        statusEl.style.color = "var(--error-color, #f44336)";
      }
...
function setupHotkeyEvents() {
  ...
  if (saveBtn) {
    ...
    saveBtn.addEventListener("click", saveHotkeySettings);
  }

  // تغيير حالة التفعيل
  const activeCheck = document.getElementById("hotkeyActive");
  if (activeCheck) {
    activeCheck.removeEventListener("change", saveHotkeySettings);
    activeCheck.addEventListener("change", saveHotkeySettings);
  }
...
function openKeyboardShortcutModal() {
  ...
  if (!modal) return;
  const currentShortcut = document.getElementById("shortcutData").value || "";
  let key = "",
    ctrl = false,
    alt = false,
    shift = false;
...
if (screensNav) {
  screensNav.onclick = function () {
    ...
    if (typeof loadScreens === "function") loadScreens();
  };
}
...
async function updateStreamerImages(force = false) {
  ...
  setTimeout(updateStreamerImages, 1000);
}
...
liveCheckInterval = setInterval(checkLiveStatus, 5000);
