...
// جذر مجلد الواجهة (الأصول: enc/ index.html images...) — الملفات التنفيذية في electron/
...
function registerAppProtocol() {
  protocol.handle("app", (req) => {
    ...
    let buffer = null;
    const encPath = path.join(APP_ROOT, "enc", p + ".enc");
    const plainPath = path.join(APP_ROOT, p);
    const hasPlain =
      fs.existsSync(plainPath) && fs.statSync(plainPath).isFile();
    // ✅ وضع التطوير (npm start): المصدر الحي أولًا — التشفير enc/ يُستخدم
    // فقط للنسخة المبنية. كان enc/ يتفوق دائماً فتشغّل npm start لقطة
    // قديمة من آخر تشفير وتضن التغييرات الجديدة "مش بتتحل"
    if (!app.isPackaged && hasPlain) {
      buffer = fs.readFileSync(plainPath);
    } else if (RESOURCE_KEY && fs.existsSync(encPath)) {
      try {
...
// ============================================================
// ✅ التحويل الذكي بين المحلي والسحابي:
// أولوية الاتصال دائماً للسيرفر المحلي (127.0.0.1:3000) إن كان حياً،
// وإلا السحابي تلقائياً — فحص كل 45 ثانية: إن تغير الأفضل يُبدَّل
// فوراً (إعادة توصيل الوكيل + تحديث الواجهة) بدون أي تدخل من المستخدم.
// التحويل في الذاكرة فقط — لا يلمس config.json حتى لا يتعارض
// مع سكربتات وضع المحلي/السحابي.
// ============================================================
const LOCAL_SERVER_URL = "http://127.0.0.1:3000";
let _failoverBusy = false;
...
async function probeServerUrl(url, timeoutMs = 1500) {
  try {
    ...
    const res = await fetch(normalizeServerUrl(url) + "/", {
      signal: ctrl.signal,
    });
    clearTimeout(timer);
...
async function resolveServerUrl() {
  if (await probeServerUrl(LOCAL_SERVER_URL)) return LOCAL_SERVER_URL;
  if (!isLocalServerUrl(config.serverUrl)) return config.serverUrl;
  return DEFAULT_SERVER_URL;
}

function startServerFailoverMonitor() {
  setInterval(async () => {
    if (_failoverBusy) return;
    _failoverBusy = true;
    try {
      const best = normalizeServerUrl(await resolveServerUrl());
      const current = normalizeServerUrl(config.serverUrl);
      if (best && best !== current) {
        config.serverUrl = best;
        logMessage(`🔀 تحويل تلقائي للسيرفر: ${best}`);
        try {
...
function getMachineId() {
  ...
  // ✅ الهوية المحفوظة هي المرجع الثابت — تُقرأ مباشرة ولا تُعاد عملية الحساب
  // (الحساب القديم كان يشتق معرفاً جديداً من المحفوظ + العتاد في كل تشغيل،
  //  فتتغير البصمة مع كل ريستارت فيراها الخادم "جهازاً آخر" ويرفض الجلسة)
  try {
    if (fs.existsSync(MACHINE_ID_FILE)) {
      const stored = fs.readFileSync(MACHINE_ID_FILE, "utf8").trim();
      if (stored && /^[a-f0-9]{32}$/i.test(stored)) {
        _machineIdCache = stored;
        return stored;
      }
    }
  } catch (_) {}
  // أول توليد فقط: المعرف القديم + طبقات العتاد المتوفرة
...
function normalizeServerUrl(url) {
  ...
  if (url.includes("localhost") || url.includes("127.0.0.1")) {
    ...
    // localhost قد يُحل إلى ::1 (IPv6) والسيرفر المحلي IPv4 فقط — نثبّت 127.0.0.1
...
function overlayStaticDirs() {
  return [
    path.join(APP_ROOT, "overlay"),
    path.join(APP_ROOT, "..", "back" + "end", "public", "overlay"),
  ];
}
...
async function executeWebhook(payload) {
  ...
  try {
    ...
    if (
      ...
      // الرابط الأصلي أولاً (النظام يحل localhost للعنوان الصحيح)،
      // ثم بدائل IPv4/IPv6 الصريحة فقط لو فشل
    ...
    for (const { url: tryUrl, family } of urlsToTry) {
      for (const tryMethod of methodsToTry) {
        try {
          ...
          // فشل محاولة وسيطة — تحذير فقط، الخطأ الحقيقي يتسجل لو كل المحاولات فشلت
          logMessage(
            `⚠️ محاولة فشلت (${tryMethod} ${tryUrl}): ${err.response?.statusText || err.message}`,
          );
          break;
        }
      }
...
function connectToServer() {
  ...
  socket.on("connect", () => {
    ...
    heartbeatInterval = setInterval(() => {
      // لو السوكيت ده لم يعد هو الرسمي (اتبدل بنسخة أحدث) — أوقف مؤقتنا فوراً
      // حتى لا يبقى مراقبان قديمان يطلقان إعادة اتصال مزدوجة
      ...
      if (Date.now() - lastEnginePacketAt > 75000) {
        logMessage("⚠️ صمت تام من السيرفر — إعادة اتصال قسرية لإعادة التسجيل");
        lastEnginePacketAt = Date.now();
        try {
          socket.disconnect();
          socket.connect();
...
app.whenReady().then(async () => {
  ...
  // ✅ تحديد السيرفر النشط قبل فتح أي نافذة: المحلي إن كان حياً وإلا السحابي
  try {
    config.serverUrl = await resolveServerUrl();
    logMessage(
      `🌐 السيرفر النشط: ${normalizeServerUrl(config.serverUrl)}`,
    );
  } catch (_) {}
  startServerFailoverMonitor();
